...
Ext.define('Shopware.apps.Article.controller.LiveShopping', {
    ...
    onStoreChanged: function() {
        ...
        if (me.getLiveShoppingDetailContainer()) {
    ...
    /**
     * @EventListener
     * Event listener function of the save button of the product detail window.
     * Fired when the user clicks the save Liveshopping button which displayed
     * in the bottom bar of the product detail window.
     */
    onSaveLiveShopping: function(options) {
        ...
        record.save({
            ...
            success: function(result, operation) {
                listing.getStore().load({
                    callback: function() {
                        ...
                        var message = '{s name=messages/saved}Live shopping product saved successfully{/s}';
            ...
            failure: function(result, operation) {
                ...
                var message = '{s name=messages/failure}An error occurded while saving the live shopping product{/s}';
     ...
     */
    onLiveShoppingTypeChanged: function() {
        var me = this;
    ...
    configurationDateChanged: function() {
        ...
        if (Ext.isDate(validFrom) && Ext.isDate(validTo) &&
            ...
            !Ext.isEmpty(configPanel.validFromTimeField.getValue()) &&
            !Ext.isEmpty(configPanel.validToTimeField.getValue())
        ) {
            // adds 1 minute
            var toTime = configPanel.validFromTimeField.getValue().getTime() + 60000;

            if (me.getRightTime(configPanel.validFromTimeField) >= me.getRightTime(configPanel.validToTimeField)) {
                // hack to set min value
                configPanel.validToTimeField.minValue = new Date(toTime);
            } else {
                configPanel.validToTimeField.setMinValue(new Date(toTime));
            }
    ...
    /**
     * Helper function which removes the date difference
     *
     * @param { Ext.form.field.Time } field
     * @returns number
     */
    getRightTime: function(field) {
        var hours = field.getValue().getHours(),
            minutes = field.getValue().getMinutes(),
            time = new Date();
            ...
            record.destroy({
                callback: function(result, operation) {
                    ...
                    if (operation.wasSuccessful()) {
                        message = '{s name=messages/delete}Live shopping product deleted.{/s}';
                        Shopware.Notification.createGrowlMessage(me.getMessageTitle(name), message);
                    } else {
                        var rawData = result.getProxy().getReader().rawData;
                        message = '{s name=messages/delete_failure}An error occurded while deleting the live shopping product{/s}';
                        if (rawData && rawData.message && rawData.message.length > 0) {
                            message = message + ':<br>' + rawData.message;
                        }
                        Shopware.Notification.createGrowlMessage(me.getMessageTitle(name), message);
        ...
        detailStore.load({
            callback: function(records, operation) {
                ...
                if (operation.wasSuccessful() && records.length > 0) {
                    ...
                    var message = '{s name=messages/load_failure}Live shopping product can not be loaded.{/s}';
                    Shopware.Notification.createGrowlMessage(me.getMessageTitle(name), message);
                }
            }
        });
    ...
    onAddPrice: function(customerGroup) {
        ...
        if (defaultPrice instanceof Ext.data.Model) {
            defaultPrice = defaultPrice.get('price');
        } else {
            defaultPrice = 1;
        }
    ...
    loadRecordIntoView: function(record) {
        ...
        try {
            ...
            var items = record.getLimitedVariants().data.items;
            if (items.length >= 1) {
                var maxValue = 0;
                for (var i = 0; i < items.length; i++) {
                    if (maxValue < items[i].data['inStock']) {
                        maxValue = items[i].data['inStock'];
                    }
                }
                configurationPanel.quantityField.maxValue = maxValue;
                configurationPanel.purchaseField.maxValue = maxValue;
            }
