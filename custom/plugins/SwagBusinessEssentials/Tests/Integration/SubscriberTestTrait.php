<?php
/**
 * Shopware Premium Plugins
 * Copyright (c) shopware AG
 *
 * According to our dual licensing model, this plugin can be used under
 * a proprietary license as set forth in our Terms and Conditions,
 * section 2.1.2.2 (Conditions of Usage).
 *
 * The text of our proprietary license additionally can be found at and
 * in the LICENSE file you have received along with this plugin.
 *
 * This plugin is distributed in the hope that it will be useful,
 * with LIMITED WARRANTY AND LIABILITY as set forth in our
 * Terms and Conditions, sections 9 (Warranty) and 10 (Liability).
 *
 * "Shopware" is a registered trademark of shopware AG.
 * The licensing of the plugin does not imply a trademark license.
 * Therefore any rights, title and interest in our trademarks
 * remain entirely with us.
 */

namespace SwagBusinessEssentails\Tests\Integration;

use Enlight_Controller_ActionEventArgs as ActionEventArgs;
use Enlight_Controller_Request_RequestTestCase as RequestTestCase;
use Enlight_Template_Manager;
use SwagBusinessEssentials\Tests\Integration\Mocks\DummyControllerMock;
use SwagBusinessEssentials\Tests\Integration\Mocks\ViewMock;

trait SubscriberTestTrait
{
    /**
     * @var ActionEventArgs
     */
    private $controllerArgs;

    /**
     * @return ActionEventArgs
     */
    public function getControllerEventArgs()
    {
        if ($this->controllerArgs) {
            return $this->controllerArgs;
        }

        $request = $this->getDummyRequest();

        return $this->controllerArgs = new ActionEventArgs([
            'subject' => $this->getDummySubject($request),
            'request' => $request,
        ]);
    }

    /**
     * @return DummyControllerMock
     */
    private function getDummySubject(RequestTestCase $request)
    {
        return new DummyControllerMock(
            $request,
            new ViewMock(new Enlight_Template_Manager())
        );
    }

    /**
     * @return RequestTestCase
     */
    private function getDummyRequest()
    {
        return new RequestTestCase();
    }
}
