...
{block name="frontend_index_content"}
    <h1>{s name='PrivateLoginWelcomeHeader' namespace='frontend/account/login'}{/s}</h1>
    {*Link for registration*}
    {block name="frontend_business_essentials_login_info"}
        <div class="business--warning-message">
            {if $optinhashinvalid && {config name=optinregister}}
                {include file="frontend/_includes/messages.tpl" type="error" content="{s name="PrivateShoppingRegisterOptinInvalidHash" namespace="frontend/account/login"}{/s}"}
            {elseif ($showOptinSentMail || $smarty.get.optinsuccess) && {config name=optinregister}}
                {include file="frontend/_includes/messages.tpl" type="success" content="{s name="PrivateShoppingRegisterOptinConfirmation" namespace="frontend/account/login"}{/s}"}
            {elseif $showConfirmation || ($showOptinSuccess && {config name=optinregister})}
