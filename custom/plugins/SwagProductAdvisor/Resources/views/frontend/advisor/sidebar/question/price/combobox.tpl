...
{block name="frontend_advisor_sidebar_answer_price_combobox"}
    <select class="advisor--price-combobox" name="q{$question['id']|escapeHtml}_values_max"{if $question['required']} required="required"{/if}>
        {block name="frontend_advisor_sidebar_price_combobox_empty"}
            <option class="advisor--empty-text" disabled="disabled"{if !$question['answered']} selected="selected"{/if}>
