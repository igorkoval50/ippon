...
class Supplier {
  ...
  protected function getEmotionsFromDb($id) {
    $sql = '
      ...
      FROM
        s_prems_emotion_cms_supplier_suppliers AS suppliers
      LEFT JOIN
        s_prems_emotion_cms_supplier AS main
      ON
        main_id = main.id
      LEFT JOIN
        s_prems_emotion_cms_supplier_emotions AS emotions
      ON
        emotions.main_id = main.id
      WHERE
        suppliers.supplier_id = ?
      AND (main.shop_id = 0 OR main.shop_id = ?)
     ';
