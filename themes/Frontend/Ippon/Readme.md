...
## Freitextfelder
- s_categories_attributes => ``cmsheadline`` - Text
- s_categories_attributes => ``cmscontent`` - Editor
- s_categories_attributes => ``highlight`` - Checkbox
- s_categories_attributes => ``separate`` - Checkbox
- s_article_attributes => ``articlegroup`` - Artikelgruppe
- s_article_attributes => ``targetgroup`` - Zielgruppe
- s_article_attributes => ``anfrageartikel`` - Checkbox
- s_article_attributes => ``sizetable`` - Text

## Emotion Templates
- ``index-full.tpl`` for negative margins
- ``index-border-bottom.tpl`` for bottom bordered

## Emotion Modifier
- ``has--padding-vertical``
- ``has--padding-horizontal``
- ``has--align-vertical`` Vertikale Zentrierung in der Einkaufswelt

## Button Modifier
- ``is--white`` Erzeugt einen weißen Button
- ``is--outline`` Erzeugt Outline Button
