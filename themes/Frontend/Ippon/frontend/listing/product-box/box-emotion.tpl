...
{block name='frontend_listing_box_article_image_picture'}
    ...
    {if $sArticle.image.thumbnails}
        ...
        <picture>
            ...
            <img class="lazyLoad" data-src="{$sArticle.image.thumbnails[0].source}" alt="{$desc|strip_tags|truncate:160}" />
        </picture>

    {elseif $sArticle.image.source}
        <img class="lazyLoad" data-src="{$sArticle.image.source}" alt="{$desc|strip_tags|truncate:160}" />
    {else}
        <img class="lazyLoad" data-src="{link file='frontend/_public/src/img/no-picture.jpg'}" alt="{$desc|strip_tags|truncate:160}" />
    {/if}
{/block}
