...
<div class="blog--box panel has--border block is--rounded">
    {block name='frontend_blog_col_blog_entry'}
        <div class="box--inner">

            {* Product box badges - highlight, newcomer, ESD product and discount *}
            {block name='frontend_blog_col_article_badges'}
                {include file="frontend/blog/blog-badges.tpl"}
            {/block}
                        ...
                        <a href="{url controller=blog action=detail sCategory=$sArticle.categoryId blogArticle=$sArticle.id}"
                           ...
                           title="{$sArticle.title|escape}">
                            {if isset($sArticle.media.thumbnails)}
                                <img class="lazyLoad" data-srcset="{$sArticle.media.thumbnails[1].sourceSet}"
                        ...
                        <div class="blog--box-description{if !$sArticle.media} is--fluid{/if}">
                            ...
                            {block name='frontend_blog_col_description_short'}
                                <div class="blog--box-description-short">
                                    {if $sArticle.shortDescription}{$sArticle.shortDescription|nl2br|truncate:300}{else}{$sArticle.shortDescription|truncate:300}{/if}
                                ...
                                {block name='frontend_blog_col_meta_data_name'}
                                    {if $sArticle.author.name}
                                        <span class="blog--metadata-author blog--metadata is--nowrap is--first">{$sArticle.author.name}</span>
                                    {/if}
                                {/block}
                            {/block}
            ...
            {block name='frontend_blog_col_meta_data'}
                <div class="blog--box-metadata panel--footer">
                    ...
                    {block name='frontend_blog_col_tags'}
                        <span class="blog--box-tags is--nowrap">
